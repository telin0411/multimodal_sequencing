...
class PreTrainedTokenizer(object):
    ...
    def encode_plus(self,
        ...
        first_ids = text[0]  # 'we propose an algorithm that <eos> we present result <eos> from motion capture data .'
    ...
    def prepare_for_model(self, ids, pair_ids=None, max_length=None, add_special_tokens=False, stride=0,
        ...
        # ids 'we propose an algorithm that <eos> we present result <eos> from motion capture data .'
        original_src = ids
        order = list(range(original_src.count('<eos>') + 1))   # [0, 1, 2, 3, 4]
        passage_length = len(order)  # 5

        sents = original_src.strip().split('<eos>')  # ['we propose an algorithm that ','we present result ','from motion capture data .']
        # print (sents)
        ...
        for sent_id1, sent_id2 in pairs_list:
            ...
            if first_index > sec_index:  # 3 1  label=0
                pairwise_label = 0
            ...
            tokenized_sent1 = tokenized_sent1[:max_length]
            tokenized_sent2 = self.tokenize(sent2, **kwargs)
            tokenized_sent2 = tokenized_sent2[:max_length]
            ...
            input_id = self.convert_tokens_to_ids(concat_sents)  # [101, 2057, 2034, 102]
            ...
            max_sample_len = len(masked_id) if max_sample_len < len(masked_id) else max_sample_len   # max pair length in this document
